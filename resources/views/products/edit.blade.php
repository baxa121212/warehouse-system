<x-app-layout>
    @php
        ...
        $title = $lang == 'ru' ? 'Редактировать товар' : ($lang == 'en' ? 'Edit Product' : 'Тауарды өңдеу');
        $subtitle = $lang == 'ru'
            ? 'Обновите информацию о товаре'
            : ($lang == 'en' ? 'Update product information' : 'Тауар туралы мәліметті жаңартыңыз');
        ...
        $updateText = $lang == 'ru' ? 'Обновить' : ($lang == 'en' ? 'Update' : 'Жаңарту');
    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-[28px] shadow-sm border border-slate-200 p-8">
                <form action="{{ route('products.update', ['product' => $product->id, 'lang' => $lang]) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4"
                                  class="w-full rounded-2xl border-slate-300 focus:border-slate-500 focus:ring-slate-500">{{ old('description', $product->description) }}</textarea>
                    </div>
                    ...
                    <div class="flex items-center gap-3 pt-4">
                        <button type="submit"
                                class="inline-flex items-center px-5 py-3 rounded-2xl bg-blue-600 text-white font-semibold hover:bg-blue-500 transition">
                            {{ $updateText }}
                        </button>
